...
flutter run
```
- عنوان الخادم مثبَّت في `lib/api.dart` (ثابت `serverUrl`) — عدّله هناك فقط إذا تغيّر
  عنوان الخادم، ثم أعد البناء.
...
| المشكلة | الحل |
|---|---|
| أندرويد: "التطبيق ليس مثبتًا" على جهاز التطوير | نسخة debug قديمة (من flutter run) بتوقيع مختلف — احذف التطبيق من الجهاز أولًا ثم ثبّت الـ release |
| نافذة Play Protect "تم حظر التطبيق" | اضغط النص الصغير **"التثبيت على أي حال"** — وليس الزر الأزرق "حسنًا" (الذي يلغي التثبيت). تحذير طبيعي لأي تطبيق خارج المتجر |
| التثبيت لا يبدأ أصلًا | الإعدادات ← التطبيقات ← وصول خاص ← "تثبيت التطبيقات غير المعروفة" ← فعّلها لتطبيق الملفات/المتصفح |
